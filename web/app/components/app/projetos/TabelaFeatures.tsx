...
export default function TabelaFeatures() {
    ...
    const featuresFiltradas = features.filter((feat) =>
        feat.titulo.toLowerCase().includes(termo.toLowerCase())
    );
        ...
        <DropdownSection label="Features" onAdd={() => setModalAberto(true)}>
            ...
            {featuresFiltradas.length > 0 ?
                (<table className="w-full text-left">
                    ...
                    <p className="px-3 text-sm text-gray-500">Nenhuma feature a exibir.</p>
                )}
        </DropdownSection>
    )
}