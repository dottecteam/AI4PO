...
function ChatBot() {
    ...
    const enviarMensagem = async () => {
        ...
        try {
            ...
            setMensagens((prev) => [...prev, { tipo: "agente", texto: "Não foi possível conectar ao agente." }])
        } finally {
            setCarregando(false)
        }
    }
        ...
        <div className="w-full h-full bg-[#010812] grid grid-rows-[10%_1fr_auto]">
            ...
            <div className="w-full bg-[#010812] flex flex-col justify-center gap-2 px-4 sm:px-6 lg:px-8 py-3">
                {anexos.length > 0 && (
                    <div className="flex gap-2 flex-wrap">
                        {anexos.map((arquivo, index) => (
                            <div key={index} className="relative w-36 h-20 bg-[#212838] rounded-lg p-3 flex flex-col justify-between">
                                <button
                                    type="button"
                                ...
                                <span className="text-xs text-[#AEC5F4] font-medium">
                                    {arquivo.name.split(".").pop()?.toUpperCase()}
                                </span>
                                <span className="text-xs text-white truncate">{arquivo.name}</span>
                            </div>
                        ))}
                    </div>
                )}
                ...
                <div className="w-full h-12 sm:h-14 flex justify-center items-center gap-3 sm:gap-6">
                    ...
                    <button type="button" onClick={enviarMensagem} disabled={carregando || (!mensagem.trim() && anexos.length === 0)} className="w-12 h-12 sm:w-16 sm:h-14 bg-[#EF7541] rounded-lg flex items-center justify-center cursor-pointer hover:bg-[#d9653a] transition-colors duration-200 shrink-0 disabled:opacity-50 disabled:cursor-not-allowed">
