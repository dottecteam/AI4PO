...
export default function PaginaProjeto() {
    ...
    const opcoesTipos: OpcaoFiltro[] = tipoUnicos.map((s) => ({ rotulo: s, valor: s }));
    ...
    const documentosFiltrados = usePesquisaEFiltro(
        documentos,
        termoComDebounce,
        (documento, termo) => documento.nome.toLowerCase().includes(termo.toLowerCase()),
        filtros,
        (documento, filtros) => {
            ...
            <DropdownSection label="Documentos" onAdd={() => uploadDocumento()}>
                <section className="flex flex-col gap-4">
                    ...
                    <section className="w-full rounded-xl bg-[#212838] border border-transparent flex flex-even items-center gap-3 p-3">
                        <Searchbar valor={termo} onAtualizar={setTermo} placeholder="Pesquise por nomes de documentos..." />
                        <MenuFiltro grupos={grupos} selecionados={{ tipo: filtros.tipo }} onAtualizar={atualizarFiltro} />
                        <FiltroData dataMin={filtros.dataMin} dataMax={filtros.dataMax} onAtualizar={atualizarData} />
                    </section>

                    {documentosFiltrados.length > 0 ? "" : (<p className="text-white text-md text-center col-span-full">Nenhum documento registrado.</p>)}
                    {documentosFiltrados.map((documento) =>
                        (<DocumentoCard documento={documento} key={documento.id} />)
                    )}
                </section>
            </DropdownSection>
        </main>
